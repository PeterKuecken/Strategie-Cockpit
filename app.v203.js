...
function speechExecuteCommand(raw){
  ...
  const navCommands=[
    [['dashboard','startseite'],()=>go('heute')],
    [['kontakte','kontaktliste'],()=>go('recruiting')],
    [['recruiting assistent','rekrutierungsassistent','assistent'],()=>go('assistent')],
    [['peter heute'],()=>go('peter_heute_home')],
    [['martina heute'],()=>go('martina_heute_home')],
    [['recruiting bibliothek','rekrutierungsbibliothek','bibliothek'],()=>go('recruiting_library_home')],
    [['vertriebscockpit','vertriebs cockpit'],()=>go('kennzahlen')],
    [['einstellungen'],()=>go('settings_home')]
  ];
  for(const [words,fn] of navCommands){if(words.some(w=>cmd===`öffne ${w}`||cmd===`oeffne ${w}`||cmd===`gehe zu ${w}`||cmd===w)){fn(); speechStatus('Befehl ausgeführt.'); return}}
  if(cmd.includes('überfällige kontakte')||cmd.includes('ueberfaellige kontakte')){go('heute'); dashboardListFilter='overdue'; render(); speechStatus('Überfällige Kontakte geöffnet.'); return}
  if(cmd.includes('heute fällige kontakte')||cmd.includes('heute faellige kontakte')){go('heute'); dashboardListFilter='today'; render(); speechStatus('Heute fällige Kontakte geöffnet.'); return}
  if(cmd.startsWith('suche ')){
    const q=raw.replace(/^suche\s+/i,'').trim(); go('recruiting'); crmFilters.q=q; crmSortMode='az'; render(); speechStatus(`Suche nach ${q} geöffnet.`); return
  }
  if(cmd.startsWith('öffne kontakt ')||cmd.startsWith('oeffne kontakt ')){
    const q=raw.replace(/^(öffne|oeffne) kontakt\s+/i,'').trim(); const c=speechFindContact(q);
    if(c){crmOpenContact(c.id); speechStatus(`${crmFullName(c)} geöffnet.`)}else speechStatus(`Kontakt „${q}“ nicht gefunden.`,true);
    return
  }
  ...
  speechStatus('Befehl nicht erkannt. Öffne die Sprachhilfe für verfügbare Befehle.',true);
}
function speechShowHelp(){
  alert(`Verfügbare Sprachbefehle:\n\n• Öffne Dashboard\n• Öffne Kontakte\n• Öffne Recruiting-Assistent\n• Öffne Peter heute\n• Öffne Martina heute\n• Öffne Recruiting-Bibliothek\n• Öffne Vertriebscockpit\n• Öffne Einstellungen\n• Zeige überfällige Kontakte\n• Zeige heute fällige Kontakte\n• Suche [Name]\n• Öffne Kontakt [Name]\n• Wiedervorlage morgen\n• Wiedervorlage nächsten Montag\n\nDas Mikrofon hört nur nach dem Antippen zu.`)
}
function renderNav(){
  nav.innerHTML='';
  const groups=window.APP_CONTENT.navGroups||[];
