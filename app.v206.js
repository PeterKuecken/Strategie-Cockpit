...
let speechTargetId='';
let speechMode='command';
let speechLastTextTargetId='';
...
function speechSupported(){return Boolean(speechApi())}
function speechStatus(message,isError=false){
  const el=document.getElementById('speechStatus');
  if(el){el.textContent=message||''; el.classList.toggle('error',Boolean(isError)); el.classList.toggle('hidden',!message)}
  if(message && typeof showCopyNotice==='function')showCopyNotice(message);
}
function speechStop(){
  try{speechRecognition?.stop()}catch(e){}
  speechActive=false;
  document.getElementById('voiceCommandBtn')?.classList.remove('listening');
  document.querySelectorAll('.voice-dictate-btn').forEach(b=>b.classList.remove('listening'));
}
function speechStart(mode='command',targetId=''){
  const Api=speechApi();
  if(!Api){speechStatus('Spracherkennung wird von diesem Browser nicht unterstützt.',true); return}
  if(speechActive){speechStop(); return}
  if(mode==='dictation' && !targetId){
    const active=document.activeElement;
    if(active && /^(INPUT|TEXTAREA)$/.test(active.tagName) && !['date','time','checkbox','radio','file','number'].includes(active.type||''))targetId=active.id||'';
    if(!targetId)targetId=speechLastTextTargetId||'';
  }
  if(mode==='dictation' && !targetId){speechStatus('Klicke zuerst in das Textfeld, in das du diktieren möchtest.',true); return}
  speechMode=mode; speechTargetId=targetId;
  const recognition=new Api();
  speechRecognition=recognition;
  recognition.lang='de-DE'; recognition.interimResults=false; recognition.continuous=false; recognition.maxAlternatives=1;
  recognition.onstart=()=>{
    speechActive=true;
    if(mode==='command')document.getElementById('voiceCommandBtn')?.classList.add('listening');
    else document.querySelector(`[data-speech-target="${targetId}"]`)?.classList.add('listening');
    speechStatus(mode==='command'?'Sprachbefehl wird aufgenommen…':'Sprich jetzt deine Notiz…');
  };
  recognition.onerror=e=>{speechStatus(e.error==='not-allowed'?'Mikrofonzugriff wurde nicht erlaubt.':'Sprache konnte nicht erkannt werden.',true)};
  recognition.onend=()=>{speechActive=false; document.getElementById('voiceCommandBtn')?.classList.remove('listening'); document.querySelectorAll('.voice-dictate-btn').forEach(b=>b.classList.remove('listening'))};
  recognition.onresult=e=>{
    const text=String(e.results?.[0]?.[0]?.transcript||'').trim();
    if(!text){speechStatus('Keine Sprache erkannt.',true); return}
    if(mode==='dictation')speechInsertText(targetId,text); else speechExecuteCommand(text);
  };
  try{recognition.start()}catch(e){speechStatus('Spracherkennung konnte nicht gestartet werden.',true)}
}
function speechInsertText(targetId,text){
  const field=document.getElementById(targetId);
  if(!field){speechStatus('Das ausgewählte Textfeld ist nicht geöffnet.',true); return}
  const existing=String(field.value||'').trim();
  field.value=existing ? `${existing}${/[.!?]$/.test(existing)?' ':'; '}${text}` : text;
  field.dispatchEvent(new Event('input',{bubbles:true}));
  field.dispatchEvent(new Event('change',{bubbles:true}));
  field.focus();
  speechLastTextTargetId=targetId;
  speechStatus('Text wurde übernommen. Bitte prüfen und anschließend speichern.');
}
function speechNormalize(text){return String(text||'').toLowerCase().replace(/[.,!?]/g,'').replace(/\s+/g,' ').trim()}
function speechFindContact(query){
  const q=crmNormalize(query);
  if(!q)return null;
  return crmContacts().find(c=>crmNormalize([crmFullName(c),c.firstName,c.lastName,c.company,c.contactCode].filter(Boolean).join(' ')).includes(q))||null;
}
function speechDateFromText(text){
  const t=speechNormalize(text); const d=new Date(); d.setHours(12,0,0,0);
  if(t.includes('übermorgen'))d.setDate(d.getDate()+2);
  else if(t.includes('morgen'))d.setDate(d.getDate()+1);
  else{
    const days={sonntag:0,montag:1,dienstag:2,mittwoch:3,donnerstag:4,freitag:5,samstag:6};
    const key=Object.keys(days).find(k=>t.includes(k));
    if(!key)return '';
    let diff=(days[key]-d.getDay()+7)%7; if(diff===0 || t.includes('nächsten') || t.includes('naechsten'))diff=diff||7;
    d.setDate(d.getDate()+diff);
  }
  return d.toISOString().slice(0,10);
}

function speechHasAny(cmd,words){return words.some(word=>cmd.includes(word))}
function speechPlanOwner(cmd){
  if(speechHasAny(cmd,['martina','martinas']))return 'Martina';
  if(speechHasAny(cmd,['peter','peters']))return 'Peter';
  if(['martina_heute_home','martina52'].includes(current))return 'Martina';
  if(['peter_heute_home','peter52'].includes(current))return 'Peter';
  return currentPerson();
}
function speechOpenAnnualPlan(owner,weekNumber=null){
  const sectionId=owner==='Martina'?'martina52':'peter52';
  current=sectionId;
  searchInput.value='';
  selectedChapterIndex=null;
  const section=sectionById(sectionId);
  if(Number.isInteger(weekNumber) && weekNumber>=1 && weekNumber<=52 && section?.chapters?.[weekNumber-1])selectedChapterIndex=weekNumber-1;
  const g=activeNavGroup(); if(g)openNavGroupLabel=g.label;
  render();
  setTimeout(()=>scrollToContent(),0);
  speechStatus(weekNumber?`${owner}: Woche ${weekNumber} geöffnet.`:`52-Wochen-Jahresplan von ${owner} geöffnet.`);
}
function speechExtractWeek(cmd){
  const match=cmd.match(/(?:woche|kw)\s*(\d{1,2})/);
  if(!match)return null;
  const week=Number(match[1]);
  return week>=1&&week<=52?week:null;
}
function speechNaturalNav(cmd){
  const planWords=['52 wochen jahresplan','52-wochen-jahresplan','52 wochen plan','jahresplan','wochenplan'];
  const week=speechExtractWeek(cmd);
  if(speechHasAny(cmd,planWords) || (week && speechHasAny(cmd,['peter','martina','jahresplan','wochenplan']))){
    speechOpenAnnualPlan(speechPlanOwner(cmd),week);
    return true;
  }
  if(week && ['peter52','martina52'].includes(current)){
    speechOpenAnnualPlan(current==='martina52'?'Martina':'Peter',week);
    return true;
  }
  const intents=[
    {terms:['dashboard','startseite','tagesübersicht','tagesuebersicht'],target:'heute',label:'Dashboard'},
    {terms:['recruiting assistent','rekrutierungsassistent','assistent'],target:'assistent',label:'Recruiting-Assistent'},
    {terms:['peter heute','peters bereich','bereich peter'],target:'peter_heute_home',label:'Peter heute'},
    {terms:['martina heute','martinas bereich','bereich martina'],target:'martina_heute_home',label:'Martina heute'},
    {terms:['recruiting bibliothek','rekrutierungsbibliothek','vorlagenbibliothek','bibliothek'],target:'recruiting_library_home',label:'Recruiting-Bibliothek'},
    {terms:['vertriebscockpit','vertriebs cockpit','auswertung','kennzahlen'],target:'kennzahlen',label:'Vertriebscockpit'},
    {terms:['einstellungen','datensicherung'],target:'settings_home',label:'Einstellungen'},
    {terms:['kontakte','kontaktliste','kontaktübersicht','kontaktuebersicht'],target:'recruiting',label:'Kontakte'}
  ];
  for(const intent of intents){
    if(speechHasAny(cmd,intent.terms)){
      go(intent.target); speechStatus(`${intent.label} geöffnet.`); return true;
    }
  }
  return false;
}

function speechExecuteCommand(raw){
  const cmd=speechNormalize(raw);
  speechStatus(`Erkannt: „${raw}“`);
  if(speechNaturalNav(cmd))return;
  if(cmd.includes('überfällige kontakte')||cmd.includes('ueberfaellige kontakte')||cmd.includes('was ist überfällig')){go('heute'); dashboardListFilter='overdue'; render(); speechStatus('Überfällige Kontakte geöffnet.'); return}
  if(cmd.includes('heute fällige kontakte')||cmd.includes('heute faellige kontakte')||cmd.includes('was ist heute fällig')){go('heute'); dashboardListFilter='today'; render(); speechStatus('Heute fällige Kontakte geöffnet.'); return}
  const openContactMatch=raw.match(/(?:öffne|oeffne|zeige|zeig mir)\s+(?:den\s+)?(?:kontakt\s+)?(.+)/i);
  if(openContactMatch && !speechHasAny(cmd,['dashboard','jahresplan','bibliothek','assistent','einstellungen','kontakte','peter heute','martina heute','vertrieb'])){
    const q=openContactMatch[1].trim(); const c=speechFindContact(q);
    if(c){crmOpenContact(c.id); speechStatus(`${crmFullName(c)} geöffnet.`); return}
  }
  const searchMatch=raw.match(/(?:suche|finde|such nach|zeige kontakte mit)\s+(.+)/i);
  if(searchMatch){
    const q=searchMatch[1].trim(); go('recruiting'); crmFilters.q=q; crmSortMode='az'; render(); speechStatus(`Suche nach ${q} geöffnet.`); return
  }
  if((cmd.includes('wiedervorlage')||cmd.includes('wieder vorlage')) && selectedContactId){
    const date=speechDateFromText(cmd); const c=crmFindContact(selectedContactId);
    if(!date||!c){speechStatus('Bitte sage zum Beispiel: Wiedervorlage morgen oder Wiedervorlage nächsten Montag.',true); return}
    if(!confirm(`Wiedervorlage für ${crmFullName(c)} auf ${formatDate(date)} setzen?`)){speechStatus('Änderung abgebrochen.'); return}
    c.followDate=date; c.updatedAt=new Date().toISOString();
    if(!Array.isArray(c.timeline))c.timeline=[];
    c.timeline.push({date:todayKey(),time:new Date().toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'}),type:'Sprachsteuerung',text:`Wiedervorlage auf ${formatDate(date)} gesetzt.`});
    save(); render(); speechStatus(`Wiedervorlage auf ${formatDate(date)} gesetzt.`); return
  }
  speechStatus('Befehl nicht erkannt. Sage zum Beispiel: Öffne den 52-Wochen-Jahresplan von Peter.',true);
}
function speechShowHelp(){
  alert(`Du kannst frei formulieren. Beispiele:

• Öffne den 52-Wochen-Jahresplan
• Zeig mir den Jahresplan von Martina
• Öffne Peter Woche 18
• Zeige Martina Woche 7
• Ich möchte zu Peter heute
• Bring mich zur Recruiting-Bibliothek
• Zeig mir die überfälligen Kontakte
• Suche Thomas Müller
• Öffne den Kontakt Thomas Müller
• Wiedervorlage nächsten Montag

Du musst den Wortlaut nicht genau einhalten. Das Mikrofon hört nur nach dem Antippen zu.`)
}
document.addEventListener('focusin',event=>{
  const el=event.target;
  if(!el || !/^(INPUT|TEXTAREA)$/.test(el.tagName))return;
  if(['date','time','checkbox','radio','file','number'].includes(el.type||''))return;
  if(!el.id)el.id=`speech_text_${Date.now()}`;
  speechLastTextTargetId=el.id;
});
...
function renderCrmForm(c){
  ...
  return `<div id="crmFormCard" class="card"><h3>${id?'Kontakt bearbeiten':'Neuen Kontakt anlegen'}</h3>
    ...
    <div class="crm-form-group"><h4>Notizen</h4><label>Notizen<div class="voice-field-wrap"><textarea id="crm_notes">${esc(c.notes||'')}</textarea><button type="button" class="voice-dictate-btn voice-field-button" data-speech-target="crm_notes" onclick="speechStart('dictation','crm_notes')">Notizen diktieren</button></div></label></div>
...
function renderCrmCommunication(c){
  ...
  return `<div class="tab-content"><h4>Kommunikation</h4>
    ${renderCrmPipeline(c)}
    ${renderCrmCommunicationBar(c)}
    <div class="crm-toolbar"><select id="crm_comm_channel">${crmHtmlOptions(crmChannelOptions(),'WhatsApp')}</select><input id="crm_comm_text" placeholder="Kurze Gesprächsnotiz"><button type="button" class="voice-dictate-btn" data-speech-target="crm_comm_text" onclick="speechStart('dictation','crm_comm_text')" title="Gesprächsnotiz diktieren">Diktieren</button><button class="primary" onclick="crmAddCommunication('${esc(c.id)}')">Notiz eintragen</button></div>
...
function renderCrmNotes(c){
  ...
  return `<div class="tab-content"><h4>Notizen</h4><div class="crm-toolbar"><input id="crm_note_new" placeholder="Neue Notiz"><button type="button" class="voice-dictate-btn" data-speech-target="crm_note_new" onclick="speechStart('dictation','crm_note_new')" title="Notiz diktieren">Diktieren</button><button class="primary" onclick="crmAddNote('${esc(c.id)}')">Notiz hinzufügen</button></div>${c.notes?`<p class="note-box">${esc(c.notes)}</p>`:''}${notes.length?notes.map(n=>`<div class="timeline-item"><strong>${esc(n.date||'')}${n.time?' · '+esc(n.time):''}</strong><br>${esc(n.text||'')}</div>`).join(''):'<p class="small">Noch keine datierten Notizen vorhanden.</p>'}<button class="copy-btn" onclick="selectedContactTab='edit'; render()">Notizen bearbeiten</button></div>`;
}
...
function appDownloadBackupFile(file,filename){
...
async function appExportBackup(){
  try{
    const payload=JSON.stringify(appBackupPayload(),null,2);
    const filename=`recruiting-cockpit-datensicherung-${todayKey()}.json`;
    const file=new File([payload],filename,{type:'application/json;charset=utf-8'});
    const mobileDevice=window.matchMedia?.('(pointer: coarse)').matches || /Android|iPhone|iPad|iPod|Mobile/i.test(navigator.userAgent||'');
    const canShareFile=mobileDevice && typeof navigator.share==='function' && (!navigator.canShare || navigator.canShare({files:[file]}));

    if(canShareFile){
      try{
        await navigator.share({
          files:[file],
          title:'Recruiting-Cockpit Datensicherung',
          text:'Datensicherung des Recruiting-Cockpits. Wähle OneDrive, Dateien, Google Drive oder einen anderen Speicherort.'
        });
        alert(`Datensicherung wurde übergeben: ${filename}`);
        return;
      }catch(shareError){
        if(shareError && shareError.name==='AbortError')return;
        console.warn('Teilen nicht möglich, Download wird verwendet.',shareError);
      }
    }

    appDownloadBackupFile(file,filename);
    alert(`Datensicherung wurde heruntergeladen: ${filename}. Du findest sie normalerweise im Ordner Downloads.`);
  }catch(e){
    console.error(e);
    alert('Die Datensicherung konnte nicht erstellt werden.');
  }
}
...
function renderSettings(s){
  ...
  view.innerHTML=`
    <div class="card settings-head"><p class="eyebrow">Version ${esc(APP_VERSION)}</p><h2>Einstellungen und Datensicherung</h2><p>Hier sicherst du alle Daten des Recruiting-Cockpits und stellst sie bei Bedarf wieder her.</p></div>
    <div class="settings-grid" id="settingsMainGrid">
      <section class="card settings-card"><h3>1. Datensicherung erstellen</h3><p>Am PC lädt die App die Sicherungsdatei in den Ordner Downloads. Auf dem Smartphone öffnet sie den Teilen-Dialog. Dort kannst du OneDrive, Dateien, Google Drive oder eine andere Speicher-App auswählen.</p><button id="settingsExportButton" class="primary" type="button" onclick="appExportBackup()">Datensicherung speichern oder teilen</button></section>
      <section class="card settings-card"><h3>2. Datensicherung einlesen</h3><p>Importiert eine zuvor exportierte Sicherungsdatei. Vor dem Überschreiben erscheint eine Sicherheitsabfrage.</p><input id="settingsBackupFile" class="hidden" type="file" accept="application/json,.json" onchange="appImportBackup(this)"><button id="settingsImportButton" class="copy-btn" type="button" onclick="appChooseBackup()">Sicherungsdatei auswählen</button></section>
      <section class="card settings-card"><h3>3. Speicherstatus</h3><p><strong>${esc(sync)}</strong></p><p>${currentUser?'Angemeldet als '+esc(currentUser.email||'Benutzer'):'Keine Cloud-Anmeldung aktiv. Die Daten liegen auf diesem Gerät.'}</p><p>Gespeicherte Kontakte: <strong>${contacts}</strong><br>Offene Aufgaben: <strong>${tasks}</strong></p></section>
      <section class="card settings-card"><h3>4. Systemstatus</h3><p>Recruiting-Bibliothek: <strong>${coverage?'vollständig zugeordnet':'Zuordnung unvollständig'}</strong></p><p>Startdatum der Jahrespläne: <strong>01.08.2026</strong></p><p>Produktivversion: <strong>${esc(APP_VERSION)}</strong></p><button class="copy-btn" type="button" onclick="go('impressum')">Technische Informationen öffnen</button></section>
    </div>`;
}
