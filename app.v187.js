...
function knowledgeAllTemplates(){return [...KNOWLEDGE_TEMPLATES,...PERSON_PHASE_TEMPLATES]}
function knowledgeTemplatesForContact(c){
  ...
  const personal=PERSON_PHASE_TEMPLATES.filter(t=>t.owner===owner && (t.phases||[]).includes(phase));
  return personal.length?personal:KNOWLEDGE_TEMPLATES;
}
function crmRecommendedTemplates(c,limit=3){return knowledgeTemplatesForContact(c).slice(0,limit)}
...
function renderKnowledge(s){
  ...
  const source=knowledgeTemplatesForContact(contact);
...
function crmAssistantSelectedDate(contactId,rec){
  ...
  return /^\d{4}-\d{2}-\d{2}$/.test(value)?value:crmAssistantSuggestedDate(crmFindContact(contactId),rec);
}
...
function crmAssistantApply(contactId){
  ...
  c.followDate=crmAssistantSelectedDate(contactId,rec);
  c.followTime='';
...
function crmRenderContactAssistant(c){
  ...
  return `<div class="process-card assistant-card contact-assistant"><div class="process-card-head"><h4>Nächste empfohlene Aktion</h4><span class="badge">${esc(crmEnsurePhase(c))}</span></div><p><strong>${esc(rec.title)}</strong></p><p class="small">${esc(rec.hint)}</p><div class="assistant-followup"><label for="crm_assistant_follow_${esc(c.id)}"><strong>Empfohlene Wiedervorlage</strong><input id="crm_assistant_follow_${esc(c.id)}" type="date" value="${esc(suggestedDate)}"></label><small>Das vorgeschlagene Datum richtet sich nach der aktuellen Recruiting-Phase. Du kannst es vor dem Übernehmen ändern.</small></div><div class="quick-actions"><button class="copy-btn" onclick="knowledgeOpenForRecommendation('${esc(c.id)}')">Alle passenden Vorlagen</button><button class="copy-btn" onclick="crmAssistantCreateTask('${esc(c.id)}',${Number(rec.days||1)})">Aufgabe anlegen</button><button class="primary" onclick="crmAssistantApply('${esc(c.id)}')">${esc(rec.button)}</button></div>${crmRenderRecommendedTemplates(c)}</div>`;
}
