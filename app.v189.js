...
function crmRenderContactAssistant(c){
  ...
  return `<div class="process-card assistant-card contact-assistant"><div class="process-card-head"><h4>Recruiting-Assistent</h4><span class="badge">${esc(crmEnsurePhase(c))}</span></div><p class="assistant-status-note"><strong>${esc(crmAssistantStatusText(c,rec))}</strong></p><p class="small">${esc(rec.hint)}</p><div class="assistant-followup"><label for="crm_assistant_follow_${esc(c.id)}"><strong>Empfohlene Wiedervorlage</strong><input id="crm_assistant_follow_${esc(c.id)}" type="date" value="${esc(suggestedDate)}" onchange="crmAssistantSetFollowDate('${esc(c.id)}',this.value)"></label><small>Du kannst das Datum vor dem Anlegen einer Aufgabe ändern. Beim Abschließen des Schritts setzt die App automatisch die Wiedervorlage für den nächsten Schritt.</small></div><div class="quick-actions"><button class="copy-btn" onclick="knowledgeOpenForRecommendation('${esc(c.id)}')">Alle passenden Vorlagen</button><button class="copy-btn" onclick="crmAssistantCreateTask('${esc(c.id)}',${Number(rec.days||1)})">Aufgabe anlegen</button><button class="primary" onclick="crmAssistantApply('${esc(c.id)}')">Schritt abschließen und weiter</button></div>${crmRenderRecommendedTemplates(c)}</div>`;
}
